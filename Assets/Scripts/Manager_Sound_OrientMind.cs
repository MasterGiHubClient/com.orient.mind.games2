...
public class Manager_Sound_OrientMind : MonoBehaviour
{
    [SerializeField] private AudioMixer mixer_OrientMind;



    public void SetVolumeMusic_OrientMind(float volume)
    {
        mixer_OrientMind.SetFloat("Music", volume <= 0.0001f ? -80f : Mathf.Log10(Mathf.Max(0.0001f, volume)) * 20);
    ...
    public void SetVolumeSFX_OrientMind(float volume)
    {
        mixer_OrientMind.SetFloat("Sound", volume <= 0.0001f ? -80f : Mathf.Log10(Mathf.Max(0.0001f, volume)) * 20);
